#!/usr/bin/env bash
# GPU 5: nonmatmul fixed fd1000 × 3 threshold modes
set -euo pipefail
...
LOG=results/_tmux_hs_nonmatmul_g5.log
BASE=(--max-samples 200 --n-warmup 10 --gamma 3.0 --seed 2026 --layer-list 0,8,16,24 --site-set nonmatmul --reuse-baseline)
mkdir -p results
exec > >(tee -a "$LOG") 2>&1
echo "[g5] start $(date -Is)"
_run() {
  local thr=$1
  shift
  echo "[g5] === fixed fd1000 thr-${thr} ==="
  "$PY" run_hellaswag_acc_sweep.py "${BASE[@]}" --fault-mode fixed --fault-delta 1000 "$@"
}
_run none --acc-no-threshold
_run golden
_run zero --acc-threshold-zero
echo "[g5] done $(date -Is)"
