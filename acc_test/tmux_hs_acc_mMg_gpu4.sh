#!/usr/bin/env bash
# GPU 4: thr-mMg + rand2pow + gamma=10
set -euo pipefail
...
LOG=results/_tmux_hs_acc_mMg_g4.log
GAMMA=10.0
BASE=(--max-samples 200 --n-warmup 10 --gamma "$GAMMA" --seed 2026 --layer-list 0,8,16,24)
mkdir -p results
set -o pipefail
{
  echo "[acc_mMg g4] start $(date -Is) CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  echo "[acc_mMg g4] === rand2pow thr-mMg gamma=${GAMMA} ==="
...
")
  ...
  echo "[acc_mMg g4] done $RUN_DIR"
  echo "[acc_mMg g4] all finished $(date -Is)"
} 2>&1 | tee -a "$LOG"
