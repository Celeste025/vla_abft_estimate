#!/usr/bin/env bash
# GPU 7: thr-mMg + fixed 1000 + gamma=10
set -euo pipefail
...
LOG=results/_tmux_hs_acc_mMg_g7.log
GAMMA=10.0
FD=1000
...
set -o pipefail
{
  echo "[acc_mMg g7] start $(date -Is) CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  echo "[acc_mMg g7] === fixed ${FD} thr-mMg gamma=${GAMMA} ==="
  "$PY" run_hellaswag_acc_sweep.py "${BASE[@]}" --fault-mode fixed --fault-delta "$FD"
  RUN_DIR=$("$PY" -c "
...
")
  "$PY" plot_sweep_summary.py \
    ...
    --title "HellaSwag ACC thr-mMg fixed+${FD} (n200 wu10 g${GAMMA} s2026)" \
    --out-png-tp-rate "$RUN_DIR/plots/sweep_tp_rate_by_layer_op.png"
  echo "[acc_mMg g7] done $RUN_DIR"
  echo "[acc_mMg g7] all finished $(date -Is)"
} 2>&1 | tee -a "$LOG"
