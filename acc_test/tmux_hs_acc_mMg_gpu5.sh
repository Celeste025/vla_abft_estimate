#!/usr/bin/env bash
# GPU 5: thr-mMg + fixed 1000 + gamma=2
set -euo pipefail
...
LOG=results/_tmux_hs_acc_mMg_g5.log
GAMMA=2.0
FD=1000
...
set -o pipefail
{
  echo "[acc_mMg g5] start $(date -Is) CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  echo "[acc_mMg g5] === fixed ${FD} thr-mMg gamma=${GAMMA} ==="
...
")
  ...
  echo "[acc_mMg g5] done $RUN_DIR"
  echo "[acc_mMg g5] all finished $(date -Is)"
} 2>&1 | tee -a "$LOG"
