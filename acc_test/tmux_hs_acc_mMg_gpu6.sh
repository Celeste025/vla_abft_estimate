#!/usr/bin/env bash
# GPU 6: thr-mMg + fixed 1000 + gamma=5
set -euo pipefail
...
LOG=results/_tmux_hs_acc_mMg_g6.log
GAMMA=5.0
FD=1000
...
set -o pipefail
{
  echo "[acc_mMg g6] start $(date -Is) CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  echo "[acc_mMg g6] === fixed ${FD} thr-mMg gamma=${GAMMA} ==="
...
")
  ...
  echo "[acc_mMg g6] done $RUN_DIR"
  echo "[acc_mMg g6] all finished $(date -Is)"
} 2>&1 | tee -a "$LOG"
