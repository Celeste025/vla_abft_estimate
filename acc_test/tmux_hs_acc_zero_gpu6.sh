...
LOG=results/_tmux_hs_acc_zero_g6.log
BASE=(--max-samples 200 --n-warmup 10 --gamma 3.0 --seed 2026 --layer-list 0,8,16,24)
mkdir -p results
set -o pipefail
{
  echo "[acc_zero g6] start $(date -Is) CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
  echo "[acc_zero g6] === fixed 10000 thr-mMz ==="
  "$PY" run_hellaswag_acc_v2_sweep.py "${BASE[@]}" --fault-mode fixed --fault-delta 10000 --acc-threshold-zero
  RUN_DIR=$("$PY" -c "
...
")
  "$PY" plot_sweep_summary.py \
    ...
    --title "HellaSwag ACC v2 thr-mMz fixed+10000 (n200 wu10 g3 s2026)" \
    --out-png-tp-rate "$RUN_DIR/plots/sweep_tp_rate_by_layer_op.png"
  echo "[acc_zero g6] done $RUN_DIR"
  echo "[acc_zero g6] all finished $(date -Is)"
} 2>&1 | tee -a "$LOG"
