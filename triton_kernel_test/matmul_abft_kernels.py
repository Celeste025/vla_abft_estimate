...
def _load_handwritten_cuda_ext():
    # Local extension built in cuda_ext_abft/ (handwritten ldmatrix+mma GEMM).
    ext_dir = Path(__file__).resolve().parent / "cuda_ext_abft"
    if str(ext_dir) not in sys.path:
        sys.path.insert(0, str(ext_dir))
    import abft_handwritten_cuda  # type: ignore

    return abft_handwritten_cuda
...
def triton_with_filter_kernel(
    ...
    # Filter in FP32 accumulator domain, then cast to FP16 for store.
    filtered = tl.where(tl.abs(accumulator) > THRESHOLD, 0.0, accumulator)
    c = filtered.to(tl.float16)
...
@triton.jit
def triton_with_filter_kernel_fixed(
    a_ptr,
    b_ptr,
    c_ptr,
    M,
...
def matmul_fixed(a, b):
    """Triton matmul with fixed config (autotune disabled)."""
    assert a.shape[1] == b.shape[0], "Incompatible dimensions"
    assert a.is_contiguous(), "Matrix A must be contiguous"
    M, K = a.shape
    _, N = b.shape
...
def matmul_with_filter_fixed(a, b, threshold=10.0):
    """Triton matmul+filter with fixed config (autotune disabled)."""
    assert a.shape[1] == b.shape[0], "Incompatible dimensions"
    assert a.is_contiguous(), "Matrix A must be contiguous"
    M, K = a.shape
    _, N = b.shape
    c = torch.empty((M, N), device=a.device, dtype=torch.float16)
    grid = (triton.cdiv(M, 128) * triton.cdiv(N, 128),)
    triton_with_filter_kernel_fixed[grid](
        a,
        b,
        c,
        M,
...
def matmul_with_filter(a, b, threshold=10.0):
    """Triton matmul with element-level filter: abs(C_ij) > threshold => 0."""
    assert a.shape[1] == b.shape[0], "Incompatible dimensions"
    assert a.is_contiguous(), "Matrix A must be contiguous"
    M, K = a.shape
    _, N = b.shape
    c = torch.empty((M, N), device=a.device, dtype=torch.float16)
    grid = lambda META: (triton.cdiv(M, META["BLOCK_SIZE_M"]) * triton.cdiv(N, META["BLOCK_SIZE_N"]),)
    triton_with_filter_kernel[grid](
        a,
        b,
        c,
        M,
...
def cuda_gemm(a, b):
    """Handwritten CUDA GEMM (TensorCore) for M=N=K=256*i."""
    assert a.shape[1] == b.shape[0], "Incompatible dimensions"
    assert a.is_contiguous(), "Matrix A must be contiguous"
    assert b.is_contiguous(), "Matrix B must be contiguous"
    ext = _load_handwritten_cuda_ext()
    c = ext.gemm_tc_sm89(a, b)
    return c
