...
from matmul_abft_kernels import (
    ...
    matmul_abft_v2c_autotune_triton_post,
    launch_matmul_naive_kernel_only,
    matmul,
    matmul_with_filter,
    matmul_fixed,
    matmul_with_filter_fixed,
    cuda_gemm,
    matmul_abft,
    matmul_abft_naive,
...
def run_benchmark(m_min=1, m_max=21, csv_out="benchmark_results.csv"):
    ...
    for i in range(m_min, m_max):
        ...
        ms_tri_filter, _, _ = triton.testing.do_bench(lambda: matmul_with_filter(a, b, threshold=10.0), quantiles=quantiles)
        ms_tri_fixed, _, _ = triton.testing.do_bench(lambda: matmul_fixed(a, b), quantiles=quantiles)
        ms_tri_filter_fixed, _, _ = triton.testing.do_bench(lambda: matmul_with_filter_fixed(a, b, threshold=10.0), quantiles=quantiles)
        ms_cuda_gemm, _, _ = triton.testing.do_bench(lambda: cuda_gemm(a, b), quantiles=quantiles)
        ...
        ms_abft_v2c_at_full_tritonpost, _, _ = triton.testing.do_bench(
            lambda: matmul_abft_v2c_autotune_triton_post(a, b)["c"], quantiles=quantiles
        )
        ...
        v2c_at_tp_out = matmul_abft_v2c_autotune_triton_post(a, b)
        # --------------------------------------------------------------------
        ...
        v2c_at_tp_full_overhead_pct = (ms_abft_v2c_at_full_tritonpost - ms_tri) / ms_tri * 100.0

        print(f"{ref_lib},{m},{n},{k},{perf(ms_ref):.4f},,,,")
        print(f"triton,{m},{n},{k},{perf(ms_tri):.4f},,,,")
        print(f"triton_filter,{m},{n},{k},{perf(ms_tri_filter):.4f},,,,")
        print(f"triton_fixed,{m},{n},{k},{perf(ms_tri_fixed):.4f},,,,")
        print(f"triton_filter_fixed,{m},{n},{k},{perf(ms_tri_filter_fixed):.4f},,,,")
        print(f"cuda_gemm,{m},{n},{k},{perf(ms_cuda_gemm):.4f},,,,")
        print(
        ...
        print(
            f"triton_abft_v2c_at_full_tritonpost,{m},{n},{k},{perf(ms_abft_v2c_at_full_tritonpost):.4f},"
            f"{v2c_at_kernel_overhead_pct:.2f},{v2c_at_tp_full_overhead_pct:.2f},"
            f"{v2c_at_tp_out['abft_abs_error'].item():.6e},{v2c_at_tp_out['abft_rel_error'].item():.6e}"
        )
        print("------------------------------------------------------------------")
