...
VARIANT_DISPATCH = {
    ...
    "abft_v1_fixed": "stage1",
    "abft_v2a": "stage1",
    "abft_v2b": "stage1",
    "abft_v2c": "stage1",
    "abft_v2c_at": "autotuned",
    "abft_v3a": "autotuned",
}
...
def run_variant(args):
    ...
    # Stage1 v2 needs different buffer shapes; allocate them once.
    if args.variant in {"abft_v1_fixed", "abft_v2c"}:
        num_pid_m_v2, num_pid_n_v2 = _v2_grid_dims(args.dim, args.dim)
        sum_a_v2 = torch.zeros((num_pid_m_v2, args.dim), device="cuda", dtype=torch.float32)
        sum_b_v2 = torch.zeros((num_pid_n_v2, args.dim), device="cuda", dtype=torch.float32)
        sum_c_v2 = torch.zeros((num_pid_m_v2, num_pid_n_v2), device="cuda", dtype=torch.float32)
    elif args.variant in {"abft_v2c_at", "abft_v3a"}:
        # Autotuned variants use bound-based partial buffer shape (matches kernel).
        num_pid_m_at, num_pid_n_at = get_abft_partial_shape_bounds(args.dim, args.dim)
        sum_a_at = torch.zeros((num_pid_m_at, args.dim), device="cuda", dtype=torch.float32)
        sum_b_at = torch.zeros((num_pid_n_at, args.dim), device="cuda", dtype=torch.float32)
        sum_c_at = torch.zeros((num_pid_m_at, num_pid_n_at), device="cuda", dtype=torch.float32)
    elif args.variant == "abft_v2a":
        num_pid_m_v2, num_pid_n_v2 = _v2_grid_dims(args.dim, args.dim)
        sum_a3d = torch.zeros((num_pid_m_v2, num_pid_n_v2, args.dim), device="cuda", dtype=torch.float32)
        sum_b3d = torch.zeros((num_pid_m_v2, num_pid_n_v2, args.dim), device="cuda", dtype=torch.float32)
        sum_c_v2 = torch.zeros((num_pid_m_v2, num_pid_n_v2), device="cuda", dtype=torch.float32)
    elif args.variant == "abft_v2b":
        num_pid_m_v2, num_pid_n_v2 = _v2_grid_dims(args.dim, args.dim)
        sum_a_total = torch.zeros((args.dim,), device="cuda", dtype=torch.float32)
        sum_b_total = torch.zeros((args.dim,), device="cuda", dtype=torch.float32)
        sum_c_v2 = torch.zeros((num_pid_m_v2, num_pid_n_v2), device="cuda", dtype=torch.float32)

    if args.variant == "triton":
        fn = lambda: _launch_triton_fixed(a, b, c, args)
    elif args.variant == "abft_v1":
        fn = lambda: _launch_abft_v1_fixed(a, b, c, sum_a, sum_b, sum_c, args)
    elif args.variant == "abft_v1_fixed":
        fn = lambda: launch_matmul_abft_v1_fixed_kernel_only(a, b, c, sum_a_v2, sum_b_v2, sum_c_v2)
    elif args.variant == "abft_v2a":
        fn = lambda: launch_matmul_abft_v2a_kernel_only(a, b, c, sum_a3d, sum_b3d, sum_c_v2)
    elif args.variant == "abft_v2b":
        def _v2b_step():
            sum_a_total.zero_()
            sum_b_total.zero_()
            launch_matmul_abft_v2b_kernel_only(a, b, c, sum_a_total, sum_b_total, sum_c_v2)
        fn = _v2b_step
    elif args.variant == "abft_v2c":
        fn = lambda: launch_matmul_abft_v2c_kernel_only(a, b, c, sum_a_v2, sum_b_v2, sum_c_v2)
    elif args.variant == "abft_v2c_at":
        fn = lambda: launch_matmul_abft_v2c_autotune_kernel_only(a, b, c, sum_a_at, sum_b_at, sum_c_at)
    elif args.variant == "abft_v3a":
        fn = lambda: launch_matmul_abft_v3a_dotsum_kernel_only(a, b, c, sum_a_at, sum_b_at, sum_c_at)
    else:
