...
from matmul_abft_kernels import (
    ...
    launch_matmul_abft_v3a_dotsum_kernel_only,
    launch_matmul_abft_v3b_latecast_kernel_only,
    matmul_abft_v2c_autotune,
    matmul_abft_v3a_dotsum,
    matmul_abft_v3b_latecast,
...
def run_benchmark(m_min=1, m_max=21, csv_out="benchmark_results.csv"):
    ...
    for i in range(m_min, m_max):
        ...
        # v2c autotune (should fix small-shape regressions vs fixed config)
        c_v2c_at = torch.empty((m, n), device="cuda", dtype=torch.float16)
        sum_a_v2c_at = torch.zeros((num_pid_m, k), device="cuda", dtype=torch.float32)
        sum_b_v2c_at = torch.zeros((num_pid_n, k), device="cuda", dtype=torch.float32)
        sum_c_v2c_at = torch.zeros((num_pid_m, num_pid_n), device="cuda", dtype=torch.float32)
        ms_abft_v2c_at_kernel, _, _ = triton.testing.do_bench(
            lambda: launch_matmul_abft_v2c_autotune_kernel_only(
                a, b, c_v2c_at, sum_a_v2c_at, sum_b_v2c_at, sum_c_v2c_at
            ),
            quantiles=quantiles,
        )
        ms_abft_v2c_at_full, _, _ = triton.testing.do_bench(
            lambda: matmul_abft_v2c_autotune(a, b)["c"], quantiles=quantiles
        )

        # v3a: HMMA-based partial sums (Stage 2)
        c_v3a = torch.empty((m, n), device="cuda", dtype=torch.float16)
        sum_a_v3a = torch.zeros((num_pid_m, k), device="cuda", dtype=torch.float32)
        sum_b_v3a = torch.zeros((num_pid_n, k), device="cuda", dtype=torch.float32)
        sum_c_v3a = torch.zeros((num_pid_m, num_pid_n), device="cuda", dtype=torch.float32)
        ms_abft_v3a_kernel, _, _ = triton.testing.do_bench(
            lambda: launch_matmul_abft_v3a_dotsum_kernel_only(
                a, b, c_v3a, sum_a_v3a, sum_b_v3a, sum_c_v3a
            ),
            quantiles=quantiles,
        )
        ms_abft_v3a_full, _, _ = triton.testing.do_bench(
            lambda: matmul_abft_v3a_dotsum(a, b)["c"], quantiles=quantiles
        )

        # v3b: late-cast tl.sum (Stage 2 alternative, lighter on registers)
        c_v3b = torch.empty((m, n), device="cuda", dtype=torch.float16)
        sum_a_v3b = torch.zeros((num_pid_m, k), device="cuda", dtype=torch.float32)
        sum_b_v3b = torch.zeros((num_pid_n, k), device="cuda", dtype=torch.float32)
        sum_c_v3b = torch.zeros((num_pid_m, num_pid_n), device="cuda", dtype=torch.float32)
        ms_abft_v3b_kernel, _, _ = triton.testing.do_bench(
            lambda: launch_matmul_abft_v3b_latecast_kernel_only(
                a, b, c_v3b, sum_a_v3b, sum_b_v3b, sum_c_v3b
            ),
            quantiles=quantiles,
        )
        ms_abft_v3b_full, _, _ = triton.testing.do_bench(
            lambda: matmul_abft_v3b_latecast(a, b)["c"], quantiles=quantiles
        )
        ...
        v2c_at_out = matmul_abft_v2c_autotune(a, b)
        v3a_out = matmul_abft_v3a_dotsum(a, b)
        v3b_out = matmul_abft_v3b_latecast(a, b)
        # --------------------------------------------------------------------
        ...
        v2c_at_kernel_overhead_pct = (ms_abft_v2c_at_kernel - ms_tri) / ms_tri * 100.0
        v2c_at_full_overhead_pct = (ms_abft_v2c_at_full - ms_tri) / ms_tri * 100.0
        v3a_kernel_overhead_pct = (ms_abft_v3a_kernel - ms_tri) / ms_tri * 100.0
        v3a_full_overhead_pct = (ms_abft_v3a_full - ms_tri) / ms_tri * 100.0
        v3b_kernel_overhead_pct = (ms_abft_v3b_kernel - ms_tri) / ms_tri * 100.0
        v3b_full_overhead_pct = (ms_abft_v3b_full - ms_tri) / ms_tri * 100.0
        ...
        print(
            f"triton_abft_v2c_at_kernel,{m},{n},{k},{perf(ms_abft_v2c_at_kernel):.4f},"
            f"{v2c_at_kernel_overhead_pct:.2f},,,"
        )
        print(
            f"triton_abft_v2c_at_full,{m},{n},{k},{perf(ms_abft_v2c_at_full):.4f},"
            f"{v2c_at_kernel_overhead_pct:.2f},{v2c_at_full_overhead_pct:.2f},"
            f"{v2c_at_out['abft_abs_error'].item():.6e},{v2c_at_out['abft_rel_error'].item():.6e}"
        )
        print(
            f"triton_abft_v3a_kernel,{m},{n},{k},{perf(ms_abft_v3a_kernel):.4f},"
            f"{v3a_kernel_overhead_pct:.2f},,,"
        )
        print(
            f"triton_abft_v3a_full,{m},{n},{k},{perf(ms_abft_v3a_full):.4f},"
            f"{v3a_kernel_overhead_pct:.2f},{v3a_full_overhead_pct:.2f},"
            f"{v3a_out['abft_abs_error'].item():.6e},{v3a_out['abft_rel_error'].item():.6e}"
        )
        print(
            f"triton_abft_v3b_kernel,{m},{n},{k},{perf(ms_abft_v3b_kernel):.4f},"
            f"{v3b_kernel_overhead_pct:.2f},,,"
        )
        print(
            f"triton_abft_v3b_full,{m},{n},{k},{perf(ms_abft_v3b_full):.4f},"
            f"{v3b_kernel_overhead_pct:.2f},{v3b_full_overhead_pct:.2f},"
            f"{v3b_out['abft_abs_error'].item():.6e},{v3b_out['abft_rel_error'].item():.6e}"
        )
        print("------------------------------------------------------------------")
